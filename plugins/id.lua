...
do
local Arian = 267713578 --put your id here(BOT OWNER ID)
local Sosha = 0 
--local Sosha2 = 267713578
...
local function run(msg, matches)
 ...
 if matches[1]:lower() == 'id' and not matches[2] then
  ...
  if msg.from.username then
   ...
   local text = text..'▶یوزر نیم️ : '..Username..'\n'
	...
	if hash then
	  ...
	  if not value then
		if msg.from.id == tonumber(Arian) then
		 text = text..'🏅مقام  : سازنده ی ربات\n'
		 elseif msg.from.id == tonumber(Sosha) then
		 text = text..'🏅مقام : ادمین ارشد ربات \n'
